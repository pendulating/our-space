...
- ✅ `app-interactive`: native Bevy map UI — click A/B to route, animated walker,
  camera dots + FOV wedges over the street network, live "cameras that saw you"
  panel. Runtime-verified (Metal/M2; renderer + meshes + egui init cleanly).
- ⏳ Phase 2 (ACE buses, dashcams, smart glasses, time), Phase 3 (batch heatmap +
  equity overlay), Phase 4 (WebGPU web embed).
...
# Headless end-to-end exposure demo (lat lon lat lon; defaults to Times Sq -> Union Sq)
cargo run -p sim-core --example route_demo -- 40.7185 -73.9890 40.7075 -74.0090
