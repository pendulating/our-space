...
- **Fixed (point-with-frustum, occlusion-tested):** for camera *c* with position `p_c`, heading `θ_c`, half-FOV `α_c`, range `R_c`, frame_rate `f_c`, schedule `s_c` — while the walker `x(t)` satisfies `|x−p_c| ≤ R_c` AND `angle(x−p_c, θ_c) ≤ α_c` AND **line-of-sight clear** (2D building-footprint segment test via NYC Building Footprints `5zhs-2jue`, uniform-grid index) AND `s_c` active: `captures += f_c · dt`. ~~Occlusion matters — Manhattan LOS is wall-limited; ignoring it grossly over-counts.~~ **Measured, and this was wrong** (2026-07-13, `docs/OCCLUSION_PLAN.md`): occlusion changes `R_i` by **0.01%** and the disparity coefficients by ≤0.002 SD. Streets are the negative space of buildings, so a street-mounted camera's sightlines to street-network points almost never cross a facade. The effect only appears past ~60 m of assumed range. Apply the 1/0.63 recall correction + uncertainty band.
